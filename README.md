...
QozeCode 是轻量级的 命令行 AI agent，基于 **LangGraph** 架构构建的专业级命令行智能体（AI Agent）。它旨在将大语言模型的推理能力与终端操作环境深度融合，通过
...
| **Moonshot**      | `Kimi K2.5` / `Kimi for Coding`   | 月之暗面双模型：K2.5通用能力强大，Coding包月模型专为编程优化 |
...
- **实时语音识别**: 依托高性能的实时语音转文本模型 (基于 Soniox)，实现准确无延迟的指令输入。
- **专有名词定制**: 支持根据当前项目环境（如代码变量、框架名等）自动校准上下文，显著提升专业术语识别率。
- **免提编码**: 彻底解放双手，在浏览代码或思考时，随时通过语音向 Agent 下达复杂的开发任务。
...
编辑配置文件，填入您的 API Key（如 OpenAI, DeepSeek, Tavily, Kimi 等）。
