...
- 🧠 **智能决策引擎**: 基于 LangGraph 状态图的 ReAct 推理框架，支持 Prompt Caching 优化
- 📋 **Plan 计划模式**: `/plan` 命令一键生成结构化执行计划，自动追踪任务进度
- 🚀 **Subagent 并行调度**: 子代理并行执行独立任务，上下文隔离，LangGraph 自动并行调度
- 🌐 **浏览器自动化**: Playwright 驱动的完整 Web 交互能力，支持键盘快捷键操作
- 🎙️ **语音交互**: 实时语音识别与语音指令
- 🖥️ **现代 TUI**: 聊天气泡式消息 UI，流式输出，实时展示 Agent 思考过程
- 🧩 **技能系统**: 可扩展的领域专家插件架构
- 🔌 **多模型支持**: 深度集成主流 LLM 厂商，支持原生深度思考与多模态视觉交互，并支持 LiteLLM 自定义端点扩展
...
| **Google**        | `Gemini 3.5 Flash` (think) / `Gemini 3.1 Pro` (think) |       ✅       | 谷歌最新多模态模型，支持原生深度思考，通过 Vertex AI 接入 |
...
| **LiteLLM**       | `GPT-5.4` / 任意兼容 OpenAI 接口的模型                         |      ✅/❌      | 自定义 API 端点扩展，其视觉支持由底层模型决定          |
...
- Prompt Caching 优化（静态/动态提示词分离，最大化缓存命中率）
- 流式输出实时展示推理过程（支持 DeepSeek reasoning_content 流式）
- 完整的工具调用链追踪
- 智能错误恢复与重试机制
- Maven/JAR 依赖排查能力（`jar tf` + `javap` 全链路分析）
- **📸 视觉（多模态）交互**: 当选用支持多模态的模型（如 `gemini-3.5-flash`）时，放置在当前项目 `.qoze/image/`
  目录下的图片会被自动读取并作为视觉上下文加载至 AI 中。这极其适用于 UI/UX 代码评审、报错日志截图分析、设计图转代码等场景。

### 📋 Plan 计划模式

QozeCode 独创的 `/plan` 命令，让 Agent 在执行复杂任务前先生成结构化的执行计划：

- **一键生成**: 输入 `/plan` 进入计划模式，Agent 自动生成三份文档
    - `01_understanding.md` — 需求理解与边界分析
    - `02_design.md` — 技术方案与关键决策
    - `03_tasks.md` — 任务详细拆分（含进度追踪）
- **自动追踪**: Agent 执行过程中通过 `update_plan_progress` 自动标记任务状态
- **持久化注入**: 计划文档像规则一样自动注入 LLM 上下文，确保执行不偏离
- **灵活调整**: 支持 `/plan status` 查看进度、`/plan clear` 清除计划、自然语言重新调整

### 🚀 Subagent 并行调度系统

QozeCode 的 Subagent 并行调度系统，让 Agent 能够将独立子任务分派给专门的子代理并行执行，大幅提升复杂任务的执行效率。

#### 核心设计

基于 LangChain 官方 Subagents 模式设计，每个子代理都拥有与主 Agent 完全一致的 **ReAct 推理循环**（LLM 推理 → 工具调用 →
观察 → 再推理），在隔离的上下文中独立完成子任务：

- **架构对标**：子代理的 Graph 架构完全对标主 Agent，采用
  `llm_call → should_continue → tool_node → should_continue_from_tool` 的标准 ReAct 循环
- **上下文隔离**：子代理不继承主 Agent 的对话历史，每次调用在全新上下文中独立执行，避免上下文膨胀
...
| 类型              | 专长        | 适用场景                  |
|-----------------|-----------|-----------------------|
| `code-explorer` | 代码探索，只读不写 | 同时探索多个模块、追踪调用链、分析项目结构 |
| `code-writer`   | 代码编写与修改   | 并行实现多个独立功能、批量修复同类问题   |
| `researcher`    | 网络搜索与信息综合 | 同时研究多个技术主题、对比不同方案     |
| `general`       | 通用任务执行    | 各类需要独立执行的子任务          |

#### 两种使用模式

**模式 1: 预定义类型** — 指定 `subagent_type`，快速分派常规任务：

```
用户: "研究 FastAPI 最佳实践，同时审查当前项目的 API 路由代码"
→ Agent 并行分派 2 个子代理：
  • researcher: 搜索 FastAPI 最新最佳实践
  • code-explorer: 审查 api/ 目录下的路由代码
```

**模式 2: 完全自定义** — 不指定类型，提供自定义 system prompt，实现完全掌控：

```
用户: "对比分析项目中的三种缓存实现方案"
→ Agent 分派一个自定义子代理，专门分析缓存方案的性能、一致性、故障恢复
```

#### 技术保障

- ⏱️ **超时保护**：每个子代理 120 秒超时限制，避免任务卡死
- 🔄 **迭代上限**：最多 120 轮推理迭代，防止无限循环
- 🛡️ **工具隔离**：子代理无浏览器、技能、计划等敏感工具权限，仅保留 14 个基础工具
- 📊 **执行统计**：每个子代理完成后输出推理轮次、工具调用次数、LLM 调用次数
...
- **键盘快捷键**: 支持 Enter、Escape、Ctrl+A、Ctrl+T 等完整键盘操作模拟
...
- **聊天气泡 UI**: 用户消息右对齐、Bot 消息左对齐，带圆形头像
- **Thinking 区域**: 实时展示模型的推理/思考过程（支持 DeepSeek、GLM-5.2 等推理模型）
- **工具状态面板**: 工具调用实时状态追踪（执行中/完成/失败），一目了然
- **自动复制**: 代码块自动复制到剪贴板，提升开发效率
- **语法高亮**: 支持多种编程语言的代码高亮
...
- `ui-ux-pro-max`: UI/UX 设计专家，支持 50+ 设计风格、21 套调色板、50 种字体搭配
- `ppt-generator`: 一键生成乔布斯风极简科技感演示稿
- `explain-code`: 可视化图表和类比解释代码
- `weather`: 实时天气与天气预报（无需 API Key）
...
- **文件操作**: 读取、写入、搜索、批量处理、目录树生成
- **命令执行**: 安全执行 Shell 命令，支持实时输出
- **网络搜索**: 集成 Tavily API，实时网络信息检索
- **URL 读取**: 网页内容提取与 Markdown 转换
- **飞书文档**: 支持读取飞书 (Lark) 文档内容

#### 高级工具

- **浏览器自动化**: 基于 Playwright 的完整 Web 交互（导航、点击、输入、键盘、多标签页）
- **语音识别**: 实时语音转文本
- **PDF 处理**: PDF 文档解析与内容提取
- **技能管理**: 激活、停用、安装技能
- **计划管理**: 生成计划、更新进度、清除计划
- **Subagent 调度**: 并行分派子代理执行独立子任务，支持 4 种预定义类型和完全自定义模式
...
- **Python 版本**: >= 3.9
- **终端环境**: 支持 True Color 的终端模拟器 (macOS推荐 iTerm2/Alacritty, Windows推荐 Windows Terminal)
- **可选依赖**: PyAudio (语音识别, 仅 macOS)
...
```powershell
irm https://raw.githubusercontent.com/KylinShinlyLin/QozeCode/main/install.ps1 | iex
```

> 💡 首次运行可能需要先执行：`Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`
...
| 平台 | 系统级路径 | 用户级路径 |
|------|-----------|-----------|
| macOS / Linux | `/etc/conf/qoze.conf` | `~/.qoze/qoze.conf` |
| Windows | `C:\ProgramData\qoze\qoze.conf` | `%APPDATA%\qoze\qoze.conf` |

首次运行时会自动生成配置文件模板，也可以手动复制：
...
```powershell
# Windows (PowerShell)
Copy-Item qoze.conf.template $env:APPDATA\qoze\qoze.conf
```
...
1. 输入 `/plan` 开启计划模式
2. 输入你的复杂任务需求
3. Agent 自动生成三份计划文档到 `.qoze/plan/`
4. 后续对话 Agent 会自动按计划执行并追踪进度
5. 输入 `/plan` 再次切换可关闭计划模式

### 5. 如何使用 LiteLLM 自定义端点？
...
然后在启动时选择 `gpt-5.4 (LiteLLM)` 即可使用自定义端点。
