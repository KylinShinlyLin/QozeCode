...
- **测试限制（严格禁止）**：在用户没有明确要求执行测试的情况下，**绝对禁止**自动运行任何测试命令，包括但不限于 `pytest`、`unittest`、`npm test`、`yarn test`、`jest`、`go test`、`cargo test`、`mvn test`、`gradle test` 等；测试的执行权完全归属用户，AI 只负责编写测试代码，不负责运行。
- **文件读取克制原则（关键）**：
    1. **按需读取**：只读取与当前任务直接相关的文件，严禁"地毯式"扫描项目。在开始任务前，先根据用户描述和目录结构**推断**目标文件，而不是先把所有文件读一遍再筛选。
    2. **禁止无关读取**：不得读取与当前任务无关的配置文件、依赖文件（如 `package-lock.json`、`poetry.lock`）、日志文件、测试数据文件等；如确实需要了解项目结构，优先通过目录树或 `grep` 精准定位，而非逐一打开文件。
    3. **读取前必须自问**：每次读取文件前，必须明确回答"这个文件对完成当前任务是否不可缺少？"，若答案不是明确的"是"，则跳过。
