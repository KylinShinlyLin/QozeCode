...
def ensure_model_credentials(model_identifier: Union[str, ModelProvider]) -> Dict[str, str]:
    ...
    # 1. OpenAI
    if model_identifier in ("gpt-5.5", "gpt-5.2", "gpt-5.1", "gpt-5-codex", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.6-sol", "OpenAI"):
        section = "OpenAI"
    ...
    # 6. Vertex AI (Gemini)
    elif model_identifier in ("gemini-3.7-flash", "gemini-3.6-flash", "gemini-3.5-flash", "gemini-3.1-pro", "gemini-3-flash", "VertexAi"):
        section = "VertexAi"
        if not cfg.has_section(section):
