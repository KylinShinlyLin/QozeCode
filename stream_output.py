...
class StreamOutput:
    ...
    async def stream_response(self, current_state, conversation_state):
        ...
        with Progress(
            ...
            task = progress.add_task(f"{CYAN}Generating.....{RESET}", total=None)
            async for message_chunk, metadata in self.agent.astream(
                ...
                if hasattr(message_chunk, 'response_metadata') and message_chunk.response_metadata:
                    if 'finish_reason' in message_chunk.response_metadata and message_chunk.response_metadata[
            ...
            progress.update(task, description=f"[bold green]✓ {CYAN}Completed{RESET}")
